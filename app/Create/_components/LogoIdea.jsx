...
function LogoIdea({formData,onHandleInputChange}) {
  ...
  const generateLogoDesignIdea=async()=>{
   ...
   setIdeas(result.data.logo_suggestions || []);
    setLoading(false);
  }
 ...
 {ideas && ideas.map((item, index) => (
  <h2
    key={index}
    onClick={() => {
      setSelectedOption(item.idea);   // 👈 idea select hoga
      onHandleInputChange(item.idea);
    }}
    className={`p-2 rounded-full border px-3 cursor-pointer
      hover:border-primary ${selectedOption === item.idea ? 'border-primary' : ''}`}
  >
    {item.idea}   {/* 👈 YEH SCREEN PE DIKHEGA */}
  </h2>
))}
      <h2 
